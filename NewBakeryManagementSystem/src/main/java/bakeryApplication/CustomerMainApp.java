...
public class CustomerMainApp {
   ...
   private static void cancelOrder(User user)
   {
      ...
      int deleteOrder =  service.cancelOrder(order,productId);
      if(deleteOrder>0)
    ...
    private static void displayAllOrders(){
        ...
        List<Order> orderList = service.displayAllOrder();

        if(orderList != null)
        {
            System.out.println("ORDER ID  \t\t  TOTAL AMOUNT ");
            for (Order order:orderList) {
                System.out.println(order.getOrderId() +"\t" +order.getTotalAmt());
            }
        }
    }
